...
def line_area_get(target_url):
	...
	for result in line_area2:
		...
		station_main = soup2.find("div", class_="contentsBodyInr")
		Prefecture_judge_list =station_main.select('ul[class="lineListA01"] > li')
		for Prefecture_judge in Prefecture_judge_list:
			Prefecture_class =  Prefecture_judge.get("class")
			Prefecture_class = "".join(Prefecture_class)
			Prefecture_class_taxt = Prefecture_judge.text
			if Prefecture_text in Prefecture_class_taxt:
				station = soup2.find("ul", class_="clearfix heightLineParent searchBtn")
				station2 = station.find_all("li", class_=Prefecture_class)
				for station_result in station2:
					station_name = station_result.text.strip()
					station_name_re = station_name.find("(")
					station_name_result = station_name[:station_name_re]
					amount_num = station_name[station_name_re:]
					station_url = station_result.a.get("href")
					station_url_result = "https://www.shamaison.com" + station_url
					list2 = []
					list2.append(Prefecture_text)
					list2.append(station_name_result)
					list2.append(amount_num)
